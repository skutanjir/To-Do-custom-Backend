...
class MathScienceExpert implements ExpertInterface
{
    ...
    private function tryArithmetic(string $msg, string $lang): ?string
    {
        ...
        if (preg_match('/\b(?:berapa|hitung|calculate|what is)\b.*?(\d+(?:\.\d+)?)\s*(?:bagi|divided|÷|\/)\s*(\d+(?:\.\d+)?)/', $msg, $m)) {
            if ((float)$m[2] == 0) {
                return $lang === 'en' ? "⚠️ Division by zero is undefined!" : "⚠️ Pembagian dengan nol tidak terdefinisi!";
            }
            $result = (float)$m[1] / (float)$m[2];
    ...
    private function tryUnitConversion(string $msg, string $lang): ?string
    {
        ...
        if (preg_match('/(\d+(?:\.\d+)?)\s*(?:jam|hour)s?\s*(?:ke|to|in|=)\s*(?:menit|minute)/i', $msg, $m)) {
            $hours = (float)$m[1];
            $minutes = $hours * 60;
            return "⏱️ {$hours} hours = **{$minutes} minutes**";
        }
        if (preg_match('/(\d+(?:\.\d+)?)\s*(?:menit|minute)s?\s*(?:ke|to|in|=)\s*(?:detik|second)/i', $msg, $m)) {
            $minutes = (float)$m[1];
            $seconds = $minutes * 60;
            return "⏱️ {$minutes} minutes = **{$seconds} seconds**";
        }
    ...
    private function tryScienceExplanation(string $msg, string $lang): ?string
    {
        $topics = [
            ...
            'atom' => [
                'id' => "⚛️ **Atom** adalah unit terkecil dari materi. Terdiri dari: Proton (+) dan Neutron (0) di inti, serta Elektron (-) mengorbit. Nomor atom = jumlah proton. Nomor massa = proton + neutron. Model atom modern menggunakan orbital probabilistik (mekanika kuantum).",
                'en' => "⚛️ **Atom** is the smallest unit of matter. Consists of: Protons (+) and Neutrons (0) in the nucleus, and Electrons (-) orbiting. Atomic number = proton count. Mass number = protons + neutrons. Modern atomic model uses probabilistic orbitals (quantum mechanics).",
            ],
            'evolusi|evolution' => [
